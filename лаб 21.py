...
class Main_Window():
    def __init__(self):
        ...
        self.ref_menu.add_command(label="Абитуриент", command=self.open_win_provider)
        self.ref_menu.add_command(label="Информация")
        self.ref_menu.add_command(label="Курсы")
        self.ref_menu.add_command(label="Пропускной балл")

        self.book_menu.add_command(label='Директор')
        self.book_menu.add_command(label='Завучи') 
        self.book_menu.add_command(label='Администрация')

        self.otch_menu.add_command(label="Отчет по поступлении абитуриентов")
        self.otch_menu.add_command(label="Отчет по документам абитуриентов")
        self.otch_menu.add_command(label="Отчет по не поступившим")
        ...
        self.main_menu.add_cascade(label="Контактные номера", menu=self.book_menu)
...
class Provider_Window():
    ...
    def __init__(self):
        ...
        self.root2.title("Книжный магазин/Абитуриенты")
...
class Provider_Window1():
    '''Окно Поставщики'''
    def __init__(self):
        self.root3 = tk.Tk()
        self.root3.geometry("800x500")
        self.root3.title("Книжный магазин/Курсы")
        self.root3.protocol('WM_DELETE_WINDOW', lambda: self.quit_win_provider()) #перехват кнопки Х
        self.main_view = win
        self.db = db
        ...
        #поля для ввода
        self.lname = tk.Label(self.add_edit_frame, text="Название курса")
        self.lname.place(relx=0.04, rely=0.02, relheight=0.05, relwidth=0.4)
        self.ename = ttk.Entry(self.add_edit_frame)
        self.ename.place(relx=0.45, rely=0.02, relheight=0.05, relwidth=0.5)

        self.lcontact = tk.Label(self.add_edit_frame, text="Описание курса")
        self.lcontact.place(relx=0.04, rely=0.12, relheight=0.05, relwidth=0.4)
        self.econtact = ttk.Entry(self.add_edit_frame)
        self.econtact.place(relx=0.45, rely=0.12, relheight=0.05, relwidth=0.5)
...
class DB:
    def __init__(self):
        ...
        #таблица Книги
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS "Абитуриент" (
